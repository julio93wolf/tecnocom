<?php
  ...
  if(isset($_REQUEST['sub'])){
    ...
    if(isset($_REQUEST['fab'])){
      ...
      $datoProductos=$tecnocom->consultar('select * from vw_oferta where id_subcategoria=:id_subcategoria and id_fabricante=:id_fabricante',$paramProducto);
    }else{
      $paramProducto['id_subcategoria'] = $_REQUEST['sub'];
      $datoProductos=$tecnocom->consultar('select * from vw_oferta where id_subcategoria=:id_subcategoria',$paramProducto);
    }
  }else{
    if(isset($_REQUEST['fab'])){
      $paramProducto['id_fabricante'] = $_REQUEST['fab'];
      $datoFabricante=$tecnocom->consultar('select * from fabricante where id_fabricante=:id_fabricante',$paramProducto);
      $datoProductos=$tecnocom->consultar('select * from vw_oferta where id_fabricante=:id_fabricante',$paramProducto);
    }else{
      $datoFabricante=$tecnocom->consultar('select id_fabricante,fabricante from fabricante where id_fabricante in (select id_fabricante from vw_oferta group by (id_fabricante)) group by fabricante');
      $datoProductos=$tecnocom->consultar('select * from vw_oferta');
    }
  }
